...
// MEMO
import { MemoFields } from "@/features/MainData/config/formFields/Memo";
...
export default function Memo() {
  ...
  const { data: memos, isLoading } = useFetchData({
    queryKey: initialData.ID ? ["memos", initialData?.ID] : ["memos"],
    axios: {
      url: "/memo",
    },
  });
  ...
  // UBAH NO_MEMO DARI SELECT JADI TEXT
  const updatedFields = MemoFields.map((field) =>
    field.name === "no_memo" ? { ...field, type: "text" } : field
  );
          ...
          <div className="flex gap-1">
            ...
            <DeleteDialog
              ...
              form={{
                id: initialData.ID,
                mutation: DeleteMemo,
                queryKey: ["memos", initialData?.ID] as string[],
              }}
            />
            <EditForm
