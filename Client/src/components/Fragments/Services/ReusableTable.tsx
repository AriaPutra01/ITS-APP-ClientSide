    ...
    return MainData.filter((item) => {
      ...
      const matchesCategory =
        ...
        selectedCategories.some((category) => {
          ...
          const noSuratMatches =
            item.no_surat && item.no_surat.includes(category);
          const codeProjectMatches =
            item.kode_project && item.kode_project.includes(category);
  ...
  const handleDeleteFile = async (id, fileName) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/${deleteFiles}/${id}/${encodedFileName}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          {uploadedFiles.map((file, index) => (
            <li key={index} className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                <Button
                  onClick={() => handleDeleteFile(selectedId, file)}
                  color="failure">
                  Hapus
                </Button>
                <Button
                  onClick={() => {
                    ...
                    if (selectedId) {
                      fetch(
                        `http://localhost:8080/${downloadFiles}/${selectedId}/${file}`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                          },
